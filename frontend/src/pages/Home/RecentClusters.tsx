...
import { getClusterPrefixedPath } from '@lib/util.ts';
import SquareButton from '@pages/Home/SquareButton.tsx';
...
function ClusterButton(props: ClusterButtonProps) {
  ...
  return <SquareButton focusRipple icon="mdi:kubernetes" label={cluster.name} ref={focusedRef} onClick={onClick} />;
}
...
export default function RecentClusters(props: RecentClustersProps) {
  ...
  return (
    <Grid aria-labelledby={`#${recentClustersLabelId}`} item container alignItems="flex-start" spacing={2}>
