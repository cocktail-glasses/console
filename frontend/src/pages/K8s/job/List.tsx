...
import { KubeContainer } from '@lib/k8s/cluster.ts';
...
export function makePodStatusLabel(job: Job) {
  ...
  const condition = job.status.conditions.find(({ status }: { status: string }) => status === 'True');
    ...
    <ResourceListView
      ...
      columns={[
        ...
        },
        {
          ...
          getValue: (job) =>
            job.status?.conditions?.find(({ status }: { status: string }) => status === 'True') ?? null,
