...
import { useSettings } from '../../Settings/hook.tsx';
...
import Ingress, { IngressBackend, IngressRule } from '@lib/k8s/ingress.ts';
    ...
    if (formatURL && urlPath) {
      return (
        <Box style={{ display: 'flex', marginBottom: '5px' }}>
          <MuiLink href={`${formatURL.protocol}//${formatURL.hostname}${urlPath}`} style={{ marginRight: '5px' }}>
            {urlPath}
          </MuiLink>
          {`(${currentPathType})`}
        </Box>
            ...
            value: (
              <LabelListItem
                labels={ingress.spec?.tls?.map(
                  (tls: { hosts: string[]; secretName: string }) => `${tls.secretName} 🞂 ${tls.hosts.join(', ')}`
                )}
              />
            ),
          },
                ...
                columns={[
                  {
                    ...
                    getter: (data: IngressRule) => <LinkStringFormat url={data.host || '*'} item={item} />,
                  },
                  {
                    label: t('translation|Path'),
                    getter: (data: IngressRule) =>
                      data.http.paths.map(({ path }) => (
                        <LinkStringFormat key={path} url={data.host || '*'} item={item} urlPath={path} />
                      )),
                  },
                  {
                    label: t('Backends'),
