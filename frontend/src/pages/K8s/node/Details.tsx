...
import { NodeTaintsLabel } from './utils.tsx';

import { ActionButton, ConfirmDialog, StatusLabelProps } from '@components/common';
import { HeaderLabel, StatusLabel, ValueLabel } from '@components/common/Label.tsx';
...
import { NameValueTable } from '@components/common/SimpleTable.tsx';
import { InlineIcon } from '@iconify/react';
import { apply, drainNode, drainNodeStatus } from '@lib/k8s/apiProxy.ts';
import { KubeMetrics } from '@lib/k8s/cluster.ts';
import Node from '@lib/k8s/node.ts';
import { getCluster, timeAgo } from '@lib/util.ts';
import { CpuCircularChart, MemoryCircularChart } from '@pages/K8s/cluster/Charts.tsx';
import { useSnackbar } from 'notistack';
import { DefaultHeaderAction } from 'redux/actionButtonsSlice.ts';
import { clusterAction } from 'redux/clusterActionSlice.ts';
...
export default function NodeDetails() {
  ...
  const [isupdatingNodeScheduleProperty, setisUpdatingNodeScheduleProperty] = useState(false);
  const [isNodeDrainInProgress, setisNodeDrainInProgress] = useState(false);
  const [nodeFromAPI, nodeError] = Node.useGet(name as string);
  ...
  return (
    <>
      ...
      <DetailsGrid
        ...
        name={name}
        error={nodeError}
        headerSection={(item) => <ChartsSection node={item} metrics={nodeMetrics} noMetrics={noMetrics} />}
        withEvents
        actions={(item) => {
  ...
  return (
    <Box py={2}>
      ...
      >
        ...
        <Grid item xs={4}>
          <MemoryCircularChart items={node && [node]} itemsMetrics={metrics} noMetrics={noMetrics} />
        </Grid>
      </Grid>
    </Box>
  );
