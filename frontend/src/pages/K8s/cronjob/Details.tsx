...
import { JobsListRenderer } from '../job/List.tsx';
import { getLastScheduleTime, getSchedule } from './List.tsx';
...
import { apply } from '@lib/k8s/apiProxy.ts';
import { KubeObjectInterface } from '@lib/k8s/cluster.ts';
...
function SpawnJobDialog(props: {
  ...
  const job = cloneDeep(cronJob.spec.jobTemplate);
  ...
  job.kind = 'Job';
  job.metadata = cloneDeep(job.metadata) || {};
  ...
  return (
    <Dialog open={openJobDialog} onClose={handleClose} aria-labelledby="form-dialog-title" maxWidth="sm">
...
export default function CronJobDetails() {
  ...
  function handleCron(cronJob: CronJob, suspend: boolean) {
    const clonedCronJob = cloneDeep(cronJob);
