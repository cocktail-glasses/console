...
import ClusterRoleBinding from '@lib/k8s/clusterRoleBinding.ts';
import RoleBinding from '@lib/k8s/roleBinding.ts';
...
export default function RoleBindingList() {
  const [bindings, setBindings] = useState<RoleBindingDict | null>(null);
  ...
  function sortBindings(kind: string) {
    return function (r1: RoleBinding, r2: RoleBinding) {
      const groups1 = r1?.subjects?.filter((subject) => subject.kind === kind).map((subject) => subject.name);
      const groups2 = r2?.subjects?.filter((subject) => subject.kind === kind).map((subject) => subject.name);
      if (groups1 && groups2) {
    ...
    <ResourceListView
      ...
      columns={[
        ...
        },
        {
          ...
          render: (item) => (
            <LabelListItem
              labels={item?.subjects?.filter((subject) => subject.kind === 'User').map((subject) => subject.name) || []}
            />
          ),
        ...
        },
        {
          ...
          render: (item) => (
            <LabelListItem
              labels={
                item?.subjects?.filter((subject) => subject.kind === 'Group').map((subject) => subject.name) || []
              }
            />
          ),
        ...
        },
        {
          ...
          render: (item) => (
            <LabelListItem
              labels={
                item?.subjects?.filter((subject) => subject.kind === 'ServiceAccount').map((subject) => subject.name) ||
                []
              }
            />
          ),
