...
import { useURLState } from '@lib/util.ts';
import { useSettings } from '@pages/Settings/hook.tsx';
...
function ColumnSortButtons(props: ColumnSortButtonProps) {
  ...
  return isDefaultSorted ? (
    ...
    <IconButton aria-label={t('translation|sort swap')} size="small" onClick={() => clickHandler(true)}>
...
function usePageURLState(key: string, prefix: string, initialPage: number): ReturnType<typeof useURLState> {
  ...
  }, [page, setZeroIndexPage]);

  useEffect(() => {
    setPage(zeroIndexPage + 1);
  }, [zeroIndexPage, setPage]);
...
export default function SimpleTable(props: SimpleTableProps) {
  ...
  const [isIncreasingOrder, setIsIncreasingOrder] = useState(!defaultSortingColumn || defaultSortingColumn > 0);
  // We use a -1 value here if no sorting should be done by default.
  const [sortColIndex, setSortColIndex] = useState(defaultSortingColumn ? Math.abs(defaultSortingColumn) - 1 : -1);
  const { t } = useTranslation();
  ...
  function handleChangeRowsPerPage(event: ChangeEvent<HTMLTextAreaElement> | ChangeEvent<HTMLInputElement>) {
                  ...
                  >
                    ...
                    {sort && (
                      <ColumnSortButtons
                        ...
                        clickHandler={(isIncreasingOrder: boolean) => sortClickHandler(isIncreasingOrder, i)}
                      />
                    )}
                  </TableCell>
                );
