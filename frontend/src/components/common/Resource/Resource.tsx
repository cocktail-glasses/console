...
import { LightTooltip, Loader, ObjectEventList } from '@components/common';
import BackLink from '@components/common/BackLink';
import Empty from '@components/common/EmptyContent.tsx';
import ErrorBoundary from '@components/common/ErrorBoundary';
import InnerTable from '@components/common/InnerTable.tsx';
import { DateLabel, HoverInfoLabel, StatusLabel, StatusLabelProps, ValueLabel } from '@components/common/Label.tsx';
import Link, { LinkProps } from '@components/common/Link.tsx';
import { metadataStyles } from '@components/common/Resource/index.tsx';
import { MainInfoSection, MainInfoSectionProps } from '@components/common/Resource/MainInfoSection';
import { MainInfoHeader } from '@components/common/Resource/MainInfoSection/MainInfoSectionHeader.tsx';
import { MetadataDictGrid, MetadataDisplay } from '@components/common/Resource/MetadataDisplay.tsx';
import PortForward from '@components/common/Resource/PortForward.tsx';
import { SectionBox } from '@components/common/SectionBox.tsx';
import SimpleTable, { NameValueTable } from '@components/common/SimpleTable.tsx';
import { Icon } from '@iconify/react';
import { labelSelectorToQuery, ResourceClasses } from '@lib/k8s';
import { ApiError } from '@lib/k8s/apiProxy.ts';
import { KubeCondition, KubeContainer, KubeContainerStatus, KubeObject, KubeObjectInterface } from '@lib/k8s/cluster.ts';
import Pod, { KubePod, KubeVolume } from '@lib/k8s/pod.ts';
import { createRouteURL, RouteURLProps } from '@lib/router.tsx';
import { getThemeName } from '@lib/themes.ts';
import { localeDate, useId } from '@lib/util.ts';
import Editor from '@monaco-editor/react';
// import { useHasPreviousRoute } from '@lib/App/RouteSwitcher';
import { PodListProps, PodListRenderer } from '@pages/K8s/pod/List.tsx';
import { Location } from 'history';
import { Base64 } from 'js-base64';
import { DefaultDetailsViewSection, DetailsViewSection } from 'redux/detailsViewSectionSlice.ts';
...
export function DetailsGrid(props: DetailsGridProps) {
  ...
  // if (!!sectionsFunc) {
  //   console.info(
  //     `Using legacy sectionsFunc in DetailsGrid for ${title || resourceType + '/' + namespace + '/' + name
...
export function PageGrid(props: PageGridProps) {
  ...
  const childrenArray = Children.toArray(children).concat(Children.toArray(sections));
  return (
...
export function ConditionsTable(props: ConditionsTableProps) {
  ...
  function getColumns() {
    const cols: {
      ...
      },
      {
        ...
        getter: (condition) =>
          condition.reason ? <HoverInfoLabel label={condition.reason} hoverInfo={condition.message} /> : '-',
      },
    ];
  ...
  return (
    <SimpleTable data={(resource && resource.status && resource.status.conditions) || []} columns={getColumns()} />
  );
}
...
export function LivenessProbes(props: { liveness: KubeContainer['livenessProbe'] }) {
  ...
  return (
    ...
    scheme: ${liveness?.httpGet?.scheme}`}
      ...
      <LivenessProbeItem>{liveness?.exec?.command && `exec[${liveness?.exec?.command.join(' ')}]`}</LivenessProbeItem>
      <LivenessProbeItem>{liveness?.successThreshold && `success = ${liveness?.successThreshold}`}</LivenessProbeItem>
      <LivenessProbeItem>{liveness?.failureThreshold && `failure = ${liveness?.failureThreshold}`}</LivenessProbeItem>
      <LivenessProbeItem>
        {liveness?.initialDelaySeconds && `delay = ${liveness?.initialDelaySeconds}s`}
      </LivenessProbeItem>
      <LivenessProbeItem>{liveness?.timeoutSeconds && `timeout = ${liveness?.timeoutSeconds}s`}</LivenessProbeItem>
      <LivenessProbeItem>{liveness?.periodSeconds && `period = ${liveness?.periodSeconds}s`}</LivenessProbeItem>
    </Box>
  );
}
...
export function ContainerInfo(props: ContainerInfoProps) {
  ...
  function ContainerStatusLabel(props: { state?: KubeContainerStatus['state'] | null; container?: KubeContainer }) {
  ...
  function StatusValue(props: { rows: { name: string; value: string | number; hide?: boolean }[] }) {
    const { rows } = props;
  ...
  function containerRows() {
    ...
    return [
      ...
      },
      {
        ...
        hide: isEmpty(container.livenessProbe),
      },
      {
        name: t('Ports'),
        value: (
...
export function ContainersSection(props: { resource: KubeObjectInterface | null }) {
  ...
  function getStatuses(statusKind: 'containerStatuses' | 'initContainerStatuses' | 'ephemeralContainerStatuses') {
  ...
  function volumeRows(volume: VolumeRowsProps['volume']) {
    ...
    function printVolumeDetails(volume: VolumeRowsProps['volume']): VolumePrints {
      ...
      for (const key in vol) {
        if (!(vol[key] === '')) {
          if (typeof vol[key] === 'string' || typeof vol[key] === 'number' || typeof vol[key] === 'boolean') {
            directPrint.push({
              volKey: key,
    ...
    return [
      ...
      ...(volumeDetails.directPrint
        ? volumeDetails.directPrint.map(({ volKey, volValue }: { volKey: string; volValue: any }) => {
            return {
              name: volKey,
              value: volValue,
            };
