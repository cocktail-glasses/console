...
import { getCluster } from '@lib/cluster.ts';
...
function storeNotifications(
  ...
  const jsonNotifications = notifications.slice(0, max).map((n) => ('toJSON' in n ? (n as Notification).toJSON() : n));
...
// TODO: ccambo, remove unused
// function mergeNotifications(oldNotifications: NotificationIface[], newNotifications: NotificationIface[]) {
...
const notificationsSlice = createSlice({
  ...
  reducers: {
     ...
     */
    // TODO: ccambo, remove unused with prefix '_'...
    setNotifications(_state, action: PayloadAction<Notification[] | NotificationIface[] | NotificationIface>) {
    ...
    updateNotifications(state, action: PayloadAction<NotificationIface[] | NotificationIface>) {
      const dispatchedNotifications = Array.isArray(action.payload) ? action.payload : [action.payload];
      ...
      let updatedState = state.notifications.map((notification) => {
        ...
        return updatedNotification ? Notification.fromJSON({ ...updatedNotification, seen: true }) : notification;
      });
