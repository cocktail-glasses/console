...
import { DetailsViewSectionType } from '@components/DetailsViewSection/DetailsViewSection.tsx';
 ...
 */
function _normalizeProcessor<Processor, ProcessorProcessor>(action: PayloadAction<Processor | ProcessorProcessor>) {
  let defailsViewSectionsProcessor: Processor = action.payload as Processor;
  if (get(defailsViewSectionsProcessor, 'id') === undefined && typeof defailsViewSectionsProcessor === 'function') {
...
const detailsViewSectionSlice = createSlice({
  ...
  reducers: {
     ...
     * If the processor is passed as a function, it will be wrapped in an object with a generated ID.
     */
    setDetailsViewSection(state, action: PayloadAction<DetailsViewSectionType | DetailsViewSection>) {
    ...
    addDetailsViewSectionsProcessor(
      state,
      action: PayloadAction<DetailsViewsSectionProcessor | DetailsViewsSectionProcessor['processor']>
    ) {
      state.detailsViewSectionsProcessors.push(
        _normalizeProcessor<DetailsViewsSectionProcessor, DetailsViewsSectionProcessor['processor']>(action)
      );
    },
  },
});
