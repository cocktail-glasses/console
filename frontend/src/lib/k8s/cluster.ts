...
import { useCluster, useConnectApi } from './index.ts';
import helpers from '@helpers';
import { createRouteURL } from '../router.tsx';
import { getCluster, timeAgo, useErrorState } from '../util.ts';
import { ApiError, apiFactory, apiFactoryWithNamespace, post, QueryParameters } from './apiProxy.ts';
import CronJob from './cronJob.ts';
import DaemonSet from './daemonSet.ts';
import Deployment from './deployment.ts';
import { KubeEvent } from './event.ts';
...
export interface KubeObjectIface<T extends KubeObjectInterface | KubeEvent> {
  ...
  useList: (opts?: ApiListOptions) => [any[], ApiError | null, (items: any[]) => void, (err: ApiError | null) => void];
  useGet: (
    name: string,
    namespace?: string
 ...
 */
export function makeKubeObject<T extends KubeObjectInterface | KubeEvent>(objectName: string): KubeObjectIface<T> {
  class KubeObject {
    ...
    static useApiList<U extends KubeObject>(
      ...
      const [objs, setObjs] = useState<{ [key: string]: U[] }>({});
    ...
    static useList<U extends KubeObject>(
      ...
      const [objList, setObjList] = useState<U[] | null>(null);
    ...
    static useGet<U extends KubeObject>(
      ...
      const [obj, setObj] = useState<U | null>(null);
      ...
      function onError(err: ApiError | null) {
        if (error === err || (!!error && !!err && error.message === err.message && error.status === err.status)) {
          return;
        }
    ...
    scale(numReplicas: number) {
      ...
      type ApiEndpointWithScale = {
        scale: {
          put: (data: { metadata: KubeMetadata; spec: { replicas: number } }, clusterName?: string) => Promise<any>;
        };
      };
