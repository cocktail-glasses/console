...
export function LogsButton({ item }: LogsButtonProps) {
  ...
  async function getRelatedPods(): Promise<Pod[]> {
    if (item instanceof Deployment || item instanceof ReplicaSet || item instanceof DaemonSet) {
      try {
        ...
        if (!labelSelector) {
          const resourceType =
            item instanceof Deployment ? 'deployment' : item instanceof ReplicaSet ? 'replicaset' : 'daemonset';
          throw new Error(t('translation|No label selectors found for this {{type}}', { type: resourceType }));
        }
  ...
  const containers = React.useMemo(() => {
    ...
    if (selectedPodIndex === 'all') return pods[0]?.spec?.containers?.map((container) => container.name) || [];
  ...
  function hasContainerRestarted(podName: string | undefined, containerName: string) {
    ...
    const cont = pod?.status?.containerStatuses?.find((c: KubeContainerStatus) => c.name === containerName);
    if (!cont) {
      return false;
    }
            ...
            (newLogs: string[]) => {
              ...
              setLogs((current) => {
                ...
                if (newLogs.length > lastLogLength) {
                  ...
                  const newLogContent = newLogs.slice(startIdx, endIdx).join('').replaceAll('\n', '\r\n');
  ...
  }, [selectedPodIndex, selectedContainer, showLogs, lines, showTimestamps, follow, clearLogs, t, pods]);
  ...
  const topActions = [
    <Box key="container-controls" sx={{ display: 'flex', gap: 2, alignItems: 'center', width: '100%' }}>
        ...
        >
          {containers.map((container) => (
            <MenuItem key={container} value={container}>
              {container}
              {hasContainerRestarted(pods[selectedPodIndex === 'all' ? 0 : selectedPodIndex]?.getName(), container) &&
      ...
      <LightTooltip
        title={
          hasContainerRestarted(
            selectedPodIndex === 'all' ? pods[0]?.getName() : pods[selectedPodIndex as number]?.getName(),
            selectedContainer
          )
