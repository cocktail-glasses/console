...
import { DateLabel, Link, Table } from '@components/common';
...
export function HelmInventory(props: Readonly<{ name: string; namespace: string }>) {
  ...
  const [secrets] = ResourceClasses.Secret.useList({
    namespace,
    ...
    if (secrets?.length > 0) {
      ...
      new Response(cs.readable)
        ...
        .then((data) => {
          ...
          const resourceKinds = helmData.chart.templates.map((template) => parseHelmTemplate(template));
          ...
          for (const resourceKind of resourceKinds) {
            if (
              ...
              !filteredResourceKinds.find(
                (item) => item.kind === resourceKind.kind && item.apiVersion === resourceKind.apiVersion
              )
            ) {
...
async function fetchResources(resourceKinds, chartName: string, namespace: string): Promise<Array<Object>> {
  const resources = [];
