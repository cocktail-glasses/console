...
import { DateLabel, Link } from '@components/common';
        ...
        },
        {
          ...
          accessorFn: (item: KubeObject) => {
            if (item.jsonData?.status) {
              return item.jsonData.status.conditions?.findIndex((c) => c.type === 'Ready') !== -1 ? 'True' : 'False';
            }
            return 'Unknown';
          },
        },
...
function inventoryNameLink(item: KubeObject) {
  ...
  // standard k8s types
  const resourceKind = ResourceClasses[kind];
  if (resourceKind) {
