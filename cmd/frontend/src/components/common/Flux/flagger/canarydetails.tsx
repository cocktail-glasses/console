...
export default function CanaryDetails() {
  ...
  return <FlaggerAvailabilityCheck>{canary && <CanaryDetailsRenderer resource={canary} />}</FlaggerAvailabilityCheck>;
}
          ...
          },
          {
            ...
            value: (
              <Stack direction="row" spacing={1}>
                {targetRef.kind && <Chip size="small" label={targetRef.kind} color="primary" variant="outlined" />}
                {targetRef.kind && (
                  <Link
                ...
                {targetRef.apiVersion && (
                  <Chip size="small" label={targetRef.apiVersion} color="default" variant="outlined" />
                )}
                  ...
                  <Box mt={1}>
                    ...
                    <Box display="flex" alignItems="center" mt={0.5}>
                      <Tooltip title="Minimum">
                        <Chip
                          size="small"
                          label={`Min: ${metric.thresholdRange.min !== undefined ? metric.thresholdRange.min : 'N/A'}`}
                          color="primary"
                          variant="outlined"
                        />
                      </Tooltip>
                      ...
                      <Tooltip title="Maximum">
                        <Chip
                          size="small"
                          label={`Max: ${metric.thresholdRange.max !== undefined ? metric.thresholdRange.max : 'N/A'}`}
                          color="secondary"
                          variant="outlined"
                        />
                      </Tooltip>
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Grid container spacing={2}>
        ...
        {analysis.stepWeights && Array.isArray(analysis.stepWeights) && analysis.stepWeights.length > 0 && (
          <Grid item xs={12}>
          ...
          <Paper key={index} variant="outlined" sx={{ p: 2 }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
              ...
              <Chip
                ...
                color={
                  webhook?.type === 'pre-rollout' ? 'warning' : webhook?.type === 'post-rollout' ? 'success' : 'default'
                }
                size="small"
              />
            </Box>
            ...
            {webhook?.metadata && Object.keys(webhook.metadata).length > 0 && (
              <Box mt={2}>
                ...
                >
                  <pre style={{ margin: 0, fontSize: '0.75rem' }}>{JSON.stringify(webhook.metadata, null, 2)}</pre>
                </Paper>
              </Box>
            )}
          </Paper>
