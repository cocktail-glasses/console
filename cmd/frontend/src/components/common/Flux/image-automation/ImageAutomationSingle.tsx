...
import { imagePolicyClass, imageRepositoriesClass, imageUpdateAutomationClass } from './ImageAutomationList';

import { ConditionsTable, Link as HeadlampLink, MainInfoSection, SectionBox, Table } from '@components/common';
...
function CustomResourceDetails(props) {
  ...
  function prepareExtraInfo() {
    ...
    if (resource.jsonData.kind === 'ImageRepository') {
      ...
      extraInfo.push({
        name: 'Exclusion List',
        value: resource.jsonData.spec?.exclusionList ? resource.jsonData.spec?.exclusionList.join(', ') : 'None',
      });
      extraInfo.push({
        ...
        value: (
          <HeadlampLink
            ...
            params={{
              ...
              namespace: resource.jsonData.spec.imageRepositoryRef.namespace ?? resource.jsonData.metadata.namespace,
  ...
  return (
    <>
      ...
      />
      {resourceClass.pluralName === imageRepositoriesClass().pluralName && <TagList resource={resource?.jsonData} />}
