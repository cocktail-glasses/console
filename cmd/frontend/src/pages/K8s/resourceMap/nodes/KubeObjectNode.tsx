...
export const KubeObjectNodeComponent = memo(({ id }: NodeProps) => {
  ...
  if (node?.nodes) {
    ...
    const warnings = node?.nodes?.filter((it) => it.kubeObject && getStatus(it.kubeObject) === 'warning')?.length ?? 0;
  ...
  const icon = kubeObject ? <KubeIcon width="42px" height="42px" kind={kubeObject.kind} /> : (node?.icon ?? null);
  ...
  return (
    ...
    >
      ...
      {node.collapsed && (node.nodes?.length ?? 0) > 0 && <CircleBadge>{node.nodes?.length ?? 0}</CircleBadge>}
