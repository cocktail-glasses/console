...
const getConnectedComponents = (nodes: GraphNode[], edges: GraphEdge[]): GraphNode[] => {
   ...
   */
  const findConnectedComponent = (node: GraphNode, componentNodes: GraphNode[], componentEdges: GraphEdge[]) => {
...
export function groupGraph(
  nodes: GraphNode[],
  edges: GraphEdge[],
  { groupBy, namespaces, k8sNodes }: { groupBy?: GroupBy; namespaces: Namespace[]; k8sNodes: Node[] }
): GraphNode {
  ...
  if (groupBy === 'namespace') {
    ...
    components.forEach((component) => {
      if (!component.kubeObject) {
        component.kubeObject = namespaces.find((namespace) => namespace.metadata.name === component.label);
        if (component.kubeObject) {
  ...
  if (groupBy === 'node') {
    ...
    components = groupByProperty(
      ...
      (component) => {
        if (component.nodes) {
          return (component.nodes.find((node) => node.kubeObject?.kind === 'Pod')?.kubeObject as Pod)?.spec?.nodeName;
        }
    ...
    components.forEach((component) => {
      if (!component.kubeObject) {
        component.kubeObject = k8sNodes.find((namespace) => namespace.metadata.name === component.label);
        if (component.kubeObject) {
          component.id = component.kubeObject.metadata.uid;
        }
      }
 ...
 * @returns
 */
export function findGroupContaining(graph: GraphNode, elementId: string, strict?: boolean): GraphNode | undefined {
