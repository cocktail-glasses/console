...
import { Badge, Box, Checkbox, Chip, CircularProgress, Popover, Stack, Typography } from '@mui/material';
...
export const GraphSourcesView = memo(
  ({ sources, sourceData, selectedSources, toggleSource }: GraphSourcesViewProps) => {
    ...
    const selected = sources.filter((source) => {
      ...
      return 'sources' in source ? source.sources.some((it) => selectedSources.has(it.id)) : isSelected;
    });
    const selectedText =
      selected.length > 2
    ...
    return (
      <>
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', width: 'fit-content', minWidth: '300px', padding: 1.5 }}>
