...
const makeOwnerRelation = (cl: KubeObjectClass): Relation => ({
  ...
  predicate(from, to) {
    ...
    return obj.metadata.ownerReferences?.find((owner) => owner.uid === to.kubeObject?.metadata?.uid) !== undefined;
  },
});
...
const configMapUsedInJobs = makeRelation(Job, ConfigMap, (job, configMap) =>
  job.spec.template.spec.volumes?.find((volume) => volume?.configMap?.name === configMap.metadata.name)
);
...
const roleBindingToServiceAccount = makeRelation(RoleBinding, ServiceAccount, (binding, sa) =>
  binding.subjects.find((subject) => subject.kind === 'ServiceAccount' && sa.metadata.name === subject.name)
);
