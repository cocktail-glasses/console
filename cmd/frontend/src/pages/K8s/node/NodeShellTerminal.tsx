...
import { DEFAULT_NODE_SHELL_LINUX_IMAGE, DEFAULT_NODE_SHELL_NAMESPACE } from '@helpers';
...
async function shell(item: Node, onExec: StreamResultsCb) {
  ...
  const linuxImage = DEFAULT_NODE_SHELL_LINUX_IMAGE;
  const namespace = DEFAULT_NODE_SHELL_NAMESPACE;
  ...
  const command = ['sh', '-c', '((clear && bash) || (clear && zsh) || (clear && ash) || (clear && sh))'];
  const tty = true;
  const stdin = true;
  const stdout = true;
  const stderr = true;
...
export function NodeShellTerminal(props: NodeShellTerminalProps) {
  ...
  function setupTerminal(containerRef: HTMLElement, xterm: XTerminal, fitAddon: FitAddon) {
    ...
    xterm.onData((data) => {
      ...
      // be good enough.
      if (data === '\u001b7' && lastKeyPressEvent?.key === '|' && lastKeyPressEvent.code === 'Digit7') {
        dataToSend = '|';
      }
  ...
  useEffect(
    () => {
      ...
      (async function () {
        ...
        const { stream, onClose } = await shell(item, (items: ArrayBuffer) => onData(xtermRef.current!, items));
