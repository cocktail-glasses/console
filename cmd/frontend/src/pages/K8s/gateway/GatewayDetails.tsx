...
import { EmptyContent, StatusLabel, StatusLabelProps } from '@components/common';
...
import Gateway, { GatewayListener, GatewayListenerStatus, GatewayStatusAddress } from '@lib/k8s/gateway';

function GatewayListenerTable(props: { listener: GatewayListener; status: GatewayListenerStatus | null }) {
                ...
                {item.getListeners().length === 0 ? (
                  ...
                  item
                    ...
                    .map((listener: GatewayListener) => (
                      <GatewayListenerTable listener={listener} status={item.getListernerStatusByName(listener.name)} />
                    ))
                )}
              </SectionBox>
            ),
