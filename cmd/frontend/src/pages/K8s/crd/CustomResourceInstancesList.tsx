...
import { Link, Loader, SectionBox, ShowHideLabel } from '@components/common/';
import Empty from '@components/common/EmptyContent';
import { ResourceListView } from '@components/common/Resource';
...
function CrInstancesView({ crds }: { crds: CRD[]; key: string }) {
  ...
  const { crInstancesList, getCRDForCR, isLoading, crdsFailedToLoad, allFailed } = useMemo(() => {
    ...
    const crKindToCRDMap = queries.reduce(
      (acc, { items }, index) => {
        if (items?.[0]) {
  ...
  return (
    <SectionBox backLink>
      ...
      <ResourceListView
        ...
        errorMessage={allFailed ? t('translation|Failed to load custom resource instances') : undefined}
