    ...
    return lists.map(({ cluster, namespace, resourceVersion }) => {
      ...
      return {
        ...
        onMessage(update: KubeListUpdateEvent<K>) {
          const key = kubeObjectListQuery<K>(kubeObjectClass, endpoint, namespace, cluster, queryParams ?? {}).queryKey;
...
export function useKubeObjectList<K extends KubeObject>({
  ...
  const [listsToWatch, setListsToWatch] = useState<{ cluster: string; namespace?: string; resourceVersion: string }[]>(
    []
  );
  ...
  // @ts-expect-error - TS compiler gets confused with iterators
  return {
