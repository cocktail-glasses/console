...
export class KubeObject<T extends KubeObjectInterface | KubeEvent = any> {
  ...
  static get apiEndpoint() {
    ...
    const factoryArguments =
      factoryArgumentsArray.length === 1 ? factoryArgumentsArray[0] : (factoryArgumentsArray as any);
  ...
  static useList<K extends KubeObject>(
    ...
    const requests = useMemo(() => {
      const clusterList = cluster ? [cluster] : clusters || (fallbackClusters.length === 0 ? [''] : fallbackClusters);

      const namespacesFromParams =
        typeof namespace === 'string' ? [namespace] : Array.isArray(namespace) ? namespace : undefined;
  ...
  static create<Args extends any[], T extends KubeObject>(this: new (...args: Args) => T, ...item: Args) {
  ...
  scale(numReplicas: number) {
    ...
    type ApiEndpointWithScale = {
      scale: {
        patch: (body: { spec: { replicas: number } }, metadata: KubeMetadata, clusterName?: string) => Promise<any>;
      };
    };
