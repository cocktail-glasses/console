...
func TestContextStore(t *testing.T) {
	...
	err := store.AddContext(&Context{Name: "test"})
	require.NoError(t, err)

	// Add another context
	err = store.AddContext(&Context{Name: "test2"})
	require.NoError(t, err)
	...
	err = store.AddContextWithKeyAndTTL(&Context{Name: "testwithttl"}, "testwithttl", 2*time.Second)
	require.NoError(t, err)
