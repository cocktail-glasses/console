...
func TestGeneratePluginPaths(t *testing.T) { //nolint:funlen
	...
	t.Run("PluginPaths", func(t *testing.T) {
		...
		pathList, err := GeneratePluginPaths("", testDirName)
		require.NoError(t, err)
	...
	t.Run("InvalidPluginPaths", func(t *testing.T) {
		...
		pathList, err := GeneratePluginPaths(fileName, "")
		assert.Error(t, err)
...
func TestHandlePluginEvents(t *testing.T) { //nolint:funlen
	...
	go HandlePluginEvents("", testDirPath, events, ch)

	// plugin list key should be empty
	pluginList, err := ch.Get(context.Background(), PluginListKey)
	require.EqualError(t, err, cache.ErrNotFound.Error())
	require.Nil(t, pluginList)

	// plugin refresh key should be empty
	pluginRefresh, err := ch.Get(context.Background(), PluginRefreshKey)
	require.EqualError(t, err, cache.ErrNotFound.Error())
	...
	for {
		_, err = ch.Get(context.Background(), PluginListKey)
		if err == nil {
			break
		}
	}
...
func TestHandlePluginReload(t *testing.T) {
	...
	err := ch.Set(context.Background(), PluginRefreshKey, true)
	require.NoError(t, err)

	// call HandlePluginReload
	HandlePluginReload(ch, w)
...
func TestDelete(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.pluginName, func(t *testing.T) {
			err := Delete(tt.pluginDir, tt.pluginName)
			if tt.expectErr {
